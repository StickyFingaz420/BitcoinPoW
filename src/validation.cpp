...
bool SignBlock(std::shared_ptr<CBlock> pblock, CWallet& wallet, const CAmount& nTotalFees, uint32_t nTime, uint32_t nNonce, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
    ...
    if (wallet.CreateCoinStake(wallet, pblock->nBits, nTotalFees, nTimeBlock, nNonce, txCoinStake, key, setCoins))
    {
