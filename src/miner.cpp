...
bool CheckStake(const std::shared_ptr<const CBlock> pblock, CWallet& wallet, ChainstateManager* chainman)
{
    ...
    BlockValidationState state;
    if (!CheckProofOfStake(::BlockIndex()[pblock->hashPrevBlock], state, *pblock->vtx[1], pblock->nBits, pblock->nTime, pblock->nNonce, proofHash, hashTarget, ::ChainstateActive().CoinsTip()))
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    ...
    std::string threadName = "BTCW_mine";
    ...
    while (true)
    {
        ...
        if (!regtestMode && !gArgs.GetBoolArg("-emergencystaking", false)) {
            while (connman->GetNodeCount(CConnman::CONNECTIONS_ALL) < 3 || ::ChainstateActive().IsInitialBlockDownload()) {
                pwallet->m_last_coin_stake_search_interval = 0;
                fTryToSync = true;
        ...
        if (setCoins.size() > 0)
        {
            ...
            for (uint32_t i=beginningTime;i<beginningTime + MAX_STAKE_LOOKAHEAD;i+=STAKE_TIMESTAMP_MASK+1) {
                ...
                if (SignBlock(pblock, *pwallet, nTotalFees, i, 0xFEEDBEEF, setCoins)) {
                    ...
                    if (SignBlock(pblockfilled, *pwallet, nTotalFees, i, 0xFEEDBEEF, setCoins)) {
                        ...
                        while (!validBlock) {
                            ...
                            if (pblockfilled->GetBlockTime() > FutureDrift(GetAdjustedTime())) {
                                if (gArgs.IsArgSet("-aggressivestaking")) {
                                    ...
                                    UninterruptibleSleep(std::chrono::milliseconds{900});
                                }
                                continue;
                            }
                            validBlock=true;
...
void StakeBTCWs(bool fStake, CWallet *pwallet, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool, boost::thread_group*& stakeThread)
{
