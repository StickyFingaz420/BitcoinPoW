...
private:
    ...
    void StakeBTCWs(bool fStake, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool);
...
public:
    ...
    bool CreateCoinStake(const CWallet &wallet, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, uint32_t nNonce, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins);
