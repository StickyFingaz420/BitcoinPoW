...
bool CWallet::CreateCoinStake(const CWallet& wallet, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, uint32_t nNonce, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setCoins)
    {
        ...
        COutPoint prevoutStake = COutPoint(pcoin.first->GetHash(), pcoin.second);
        if (CheckKernel(pindexPrev, nBits, nTimeBlock, nNonce, prevoutStake, ::ChainstateActive().CoinsTip(), stakeCache))
        {
            // Found a kernel
...
int CWalletTx::GetBlocksToMaturity() const
{
    ...
    //BTCWTODO Some wallets are still not abandoning coinstakes correctly
...
void CWallet::StakeBTCWs(bool fStake, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    ::StakeBTCWs(fStake, this, connman, chainman, mempool, stakeThread);
}

void CWallet::StartStake(CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    m_enabled_staking = true;
    StakeBTCWs(true, connman, chainman, mempool);
}
